...
import com.vk.edu.LinearInterpolation.interpolateArray
import com.vk.edu.WaveRepository.waveData
...
class WaveView @JvmOverloads constructor(
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        Log.d("taag", "" +
            "width mode exactly = ${(MeasureSpec.getMode(widthMeasureSpec) == MeasureSpec.EXACTLY)}\n" +
            "width mode at_most = ${(MeasureSpec.getMode(widthMeasureSpec) == MeasureSpec.AT_MOST)}" +
            "")

        // считаем что расстояние между столбиками равно ширине столбиков
        val desiredWidth = originalData.size * itemWidth * 2 - itemWidth + paddingStart + paddingEnd
        val actWidth = resolveSize(desiredWidth, widthMeasureSpec)

        val actItemCount = (measuredWidth - paddingStart - paddingEnd + itemWidth) / (itemWidth * 2)
        measuredData = interpolateArray(originalData, actItemCount) // сокращаем исходный массив до нужного

        setMeasuredDimension(actWidth, height)
    }
